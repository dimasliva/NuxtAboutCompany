...
<!-- 4:33 -->
        <tbody>
            <tr v-for="topic in topics" :key="topic.id" :class="[styles.topicListItem, styles.trTd, styles.pinned]">
                <td :class="[styles.mainLink]">
                    <span :class="[styles.linkTopLine, styles.topicListData]">
                        ...
                        <span :class="[styles.tdTheme]">
                            <NuxtLink :to="topic.link" :class="[styles.rawLink]">
                                {{ topic.theme }}
                    ...
                    <div :class="[styles.linkBottomLine]">
                        <NuxtLink :class="[styles.categoryWrapper]" :to="topic.categoryLink">
                            <span :class="[styles.categoryName]">{{topic.category}}</span>
                        </NuxtLink>
                    </div>
                </td>
                <td :class="[styles.authorTd]">
                    <div class="creator">
                        <NuxtLink :class="[styles.creatorLink]" :to="topic.authorLink">
                            {{ topic.author }}
                        </NuxtLink>
                    </div>
                </td>
                ...
                <td :class="[styles.topicListData, styles.num, styles.tdWAuto]">
                    <span :class="[styles.tdNumber]" title="this topic has been viewed {{ topic.views }} times">{{
                        (topic.views / 1000).toFixed(1) }}k</span>
                </td>
                ...
                <td title="Created: Nov 15, 2024 6:06 am Latest: Mar 21, 2025 2:23 am"
                    ...
                    <NuxtLink :to="`/en/blizzard/t/${topic.theme.replace(/\s+/g, '-').toLowerCase()}/`"
                        ...
                        <span class="relative-date">{{ topic.activity }}</span>
                    </NuxtLink>
                </td>
...
const { topics, truncateText,  } = useForumTable();
